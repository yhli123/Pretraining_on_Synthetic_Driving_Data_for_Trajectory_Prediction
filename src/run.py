...
def learning_rate_decay(args, i_epoch, optimizer, optimizer_2=None):
    ...
    if 'set_predict' in args.other_params:
        ...
        # if i_epoch % 5 == 2: # Backup code for continuing FT
...
def train_one_epoch(model, iter_bar, optimizer, device, args: utils.Args, i_epoch, queue=None, optimizer_2=None):
    ...
    for step, batch in enumerate(iter_bar):
        if 'data_ratio_per_epoch' in args.other_params:
            ...
            if max_iter_num == 0:
                break
        if args.pretrain_label:
            # Mix Masking
            
            # if args.other_params['mask'] == 'map':
            #     mask_option = 'map'
            # elif args.other_params['mask'] == 'traj':
            #     mask_option = 'traj'
            # elif args.other_params['mask'] == 'mix':
            #     Mask_Policy = np.random.rand()
            #     if Mask_Policy < 0.90:
            #         mask_option = 'map'
            #     else:
            #         mask_option = 'traj'
            loss = model(batch, device)
            writer.add_scalar("PretrainLoss", loss, step + i_epoch*len(iter_bar))
        else:
            loss, DE, _ = model(batch, device)
            writer.add_scalar("FinetuneLoss", loss, step + i_epoch*len(iter_bar))
        loss.backward()
        if is_main_device(device):
            if args.pretrain_label:
    ...
    if not args.debug and is_main_device(device):
        ...
        if args.pretrain_label:
            output_model_file = os.path.join(
                args.model_save_dir, "model.pretrain.{0}.bin".format(i_epoch + 1))
        else:
            output_model_file = os.path.join(
    ...
    if not args.debug and is_main_device(device):
        optim1_to_save = optimizer
        optim2_to_save = optimizer_2
        if args.pretrain_label:
            output_optim_file = os.path.join(
                args.model_save_dir, "optim.pretrain.{0}.bin".format(i_epoch + 1))
        else:
            output_optim_file = os.path.join(
                args.model_save_dir, "optim.{0}.bin".format(i_epoch + 1))
        torch.save([optim1_to_save.state_dict(), optim2_to_save.state_dict()], output_optim_file)
...
def demo_basic(rank, world_size, kwargs, queue):
    ...
    if world_size > 0:
        ...
        if args.pretrain_label:
            print('Pre-Training...')
            model = pretrain.MaskedVectorNet(args).to(rank)
            # Backup code for continuing pt
            # model.load_state_dict \
            #    (torch.load(os.path.join(args.model_save_dir, "model.pretrain.10.bin")),
            #     strict=True)
            ...
            model.load_state_dict \
                (torch.load(os.path.join(args.model_save_dir, "model.pretrain.{0}.bin".format(args.other_params['use_pt_model']))),
                 strict=False)
            print('Pretrain Model Loaded Successfully!')
            # Backup code for continuing ft
            # model.load_state_dict \
            #    (torch.load(os.path.join(args.model_save_dir, "model.13.bin")),
            #     strict=True)
    ...
    if args.pretrain_label:
        optimizer = torch.optim.Adam(
            [each[1] for each in model.named_parameters() if not str(each[0]).startswith('module.pred')],
            lr=args.learning_rate)
        optimizer_2 = torch.optim.Adam(
            [each[1] for each in model.named_parameters() if str(each[0]).startswith('module.pred')],
            lr=args.learning_rate)
        # Backup code for continuing PT
        # optimizer.load_state_dict \
        #        (torch.load(os.path.join(args.model_save_dir, "optim.pretrain.10.bin"))[0])
        # optimizer_2.load_state_dict \
        #        (torch.load(os.path.join(args.model_save_dir, "optim.pretrain.10.bin"))[1])
        # print('Continuing PT opt loaded.')
    elif 'set_predict' in args.other_params:
        optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=args.learning_rate)
    elif 'complete_traj-3' in args.other_params:
        optimizer = torch.optim.Adam(
            [each[1] for each in model.named_parameters() if
             not str(each[0]).startswith('module.decoder.complete_traj')],
            lr=args.learning_rate)
        optimizer_2 = torch.optim.Adam(
        ...
        # Backup code for continuing FT
        # optimizer.load_state_dict \
        #        (torch.load(os.path.join(args.model_save_dir, "optim.13.bin"))[0])
        # optimizer_2.load_state_dict \
        #        (torch.load(os.path.join(args.model_save_dir, "optim.13.bin"))[1])
        # print('Continuing FT opt loaded.')
    else:
    ...
    for i_epoch in range(int(args.num_train_epochs)):
        args.cur_epoch = i_epoch
        args.fig_name = 0
        if args.pretrain_label:
            pass # No LR decay for pretraining
        elif 'complete_traj-3' in args.other_params:
        ...
        if 'complete_traj-3' in args.other_params:
            # Always this side for our program
...
def main():
    ...
    # Pretrain-related args
    # 'pretrain' indicates whether doing pretraining *or* fine-tuning.
    # Its number indicates the mask ratio for *map* reconstruction.
    if 'pretrain' in args.other_params:
        args.pretrain_label = True
    else:
        args.pretrain_label = False
    ...
    if args.argoverse:
        if args.do_train:
            ...
            # Eval
            from do_eval import do_eval # DenseTNT wrote this.
            do_eval(args)
    else:
        assert False
