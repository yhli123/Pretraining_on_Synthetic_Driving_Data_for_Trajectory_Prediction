...
class GlobalGraph(nn.Module):
    ...
    def __init__(self, hidden_size, attention_head_size=None, num_attention_heads=1, qkv_bias=False):
        ...
        self.query = nn.Linear(hidden_size, self.all_head_size * self.num_qkv, bias=qkv_bias)
        self.key = nn.Linear(hidden_size, self.all_head_size * self.num_qkv, bias=qkv_bias)
        self.value = nn.Linear(hidden_size, self.all_head_size * self.num_qkv, bias=qkv_bias)
...
class CrossAttention(GlobalGraph):
    def __init__(self, hidden_size, attention_head_size=None, num_attention_heads=1, qkv_bias=False, key_hidden_size=None,
        ...
        if query_hidden_size is not None:
            self.query = nn.Linear(query_hidden_size, self.all_head_size * self.num_qkv, bias=qkv_bias)
        if key_hidden_size is not None:
            self.key = nn.Linear(key_hidden_size, self.all_head_size * self.num_qkv, bias=qkv_bias)
            self.value = nn.Linear(key_hidden_size, self.all_head_size * self.num_qkv, bias=qkv_bias)
...
class GlobalGraphRes(nn.Module):
    def __init__(self, hidden_size, qkv_bias=False):
        super(GlobalGraphRes, self).__init__()
        self.global_graph = GlobalGraph(hidden_size, hidden_size // 2, qkv_bias=qkv_bias)
        self.global_graph2 = GlobalGraph(hidden_size, hidden_size // 2, qkv_bias=qkv_bias)
...
class CrossAttentionRes(nn.Module):
    def __init__(self, hidden_size, qkv_bias=False):
        super(CrossAttentionRes, self).__init__()
        self.cross_att_1 = CrossAttention(hidden_size, hidden_size // 2, qkv_bias=qkv_bias)
        self.cross_att_2 = CrossAttention(hidden_size, hidden_size // 2, qkv_bias=qkv_bias)

    def forward(self, hidden_states_query, hidden_states_key, attention_mask=None, mapping=None):
        # hidden_states = self.global_graph(hidden_states, attention_mask, mapping) \
        #                 + self.global_graph2(hidden_states, attention_mask, mapping)
        hidden_states = torch.cat([self.cross_att_1(hidden_states_query, hidden_states_key, attention_mask, mapping),
                                   self.cross_att_2(hidden_states_query, hidden_states_key, attention_mask, mapping)], dim=-1)
        return hidden_states
