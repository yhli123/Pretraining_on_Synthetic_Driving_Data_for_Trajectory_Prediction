...
class MaskedVectorNet(nn.Module):
    ...
    def initialize_traj_pos_weights(self):
        ...
        # print('Hidden States =', hidden_size)
        ...
        # assert False, 'Check Point for Pos Enc'
    ...
    def mask(self, input_list, mask_ratio, map_start_polyline_idx, device):
        ...
        if mask_option == 'map':
            ...
            ids_shuffle = torch.argsort(noise, dim=0)  # ascend: small is kept, large is removed
        ...
        for id in range(len(input_list)):
            if id in ids_target:
                ...
                if mask_option == 'traj':
                    ...
                    # assert input_list[id].shape[0] == 9, 'Map length is not 9!'
    ...
    def forward(self, mapping: List[Dict], device):
        ...
        # Global Graph Input
        # print('After Sub Shape =',element_states_batch[0].shape)
        ...
        for i, length in enumerate(inputs_lengths):
            ...
            # Just padding mask here, as masked tokens are absent.
        ...
        # Test
        ##############################################################################
        # print('For all batch from here.')
        # print('Kept Hidden States Shape = ',hidden_states.shape)
        ##############################################################################
        # Decoder 1: Cross Attention
        # print('Target List Shape =',target_masked[0].shape)

        ###################################################### JUST FOR TRAJ ONLY!!!!! ############################################################
        # print('Target List Shape =', len(target_masked), target_masked[0].shape)
        ...
        # print('Target Tensor Shape =', target_queries.shape)
        ##############################################################################
        # print('Reconstructed Hidden States Shape = ',output_hidden_states.shape)
        ##############################################################################
        ...
        # print('Pred Traj Shape =',target_pred.shape)
        ...
        for i in range(batch_size):
            for k in range(len(ids_target_list[i])):
                if mask_option_list[i] == 'traj':
                    ...
                    # print('Traj Loss Array =',loss_array)
