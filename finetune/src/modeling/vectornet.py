    ...
    def forward_encode_sub_graph(self, mapping: List[Dict], matrix: List[np.ndarray], polyline_spans: List[List[slice]],
        ...
        if 'laneGCN' in args.other_params:
            for i in range(batch_size):
                ...
                # # TEMP HOLD
                # L2L = self.laneGCN_A2L(lanes.unsqueeze(0),lanes.unsqueeze(0)).squeeze(0)
                # A2L = self.laneGCN_A2L(lanes.unsqueeze(0),agents[0:1].unsqueeze(0)).squeeze(0)
                # TOT = self.laneGCN_A2L(lanes.unsqueeze(0), torch.cat([lanes, agents[0:1]]).unsqueeze(0)).squeeze(0)
                # print('L2L shape =',L2L.shape, 'A2L shape =', A2L.shape, 'TOT shape =', TOT.shape)
                # print('Lane =', torch.abs(lanes).sum().item(), 'L2L', torch.abs(L2L).sum().item(), 'A2L', torch.abs(A2L).sum().item(), \
                #       'TOT', torch.abs(TOT).sum().item())
                # # print('Lane Value =', lanes.mean().item())
                # GCN_Value = self.laneGCN_A2L(lanes.unsqueeze(0),
                #                                  torch.cat([lanes, agents[0:1]]).unsqueeze(0)).squeeze(0)
                # print('Lane Value =', torch.abs(lanes).mean().item(), 'GCN Value =', torch.abs(GCN_Value).mean().item())
                # # assert False
